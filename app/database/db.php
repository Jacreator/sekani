...
function paginatePosts($count, $page = 1, $filter = [])
{
    global $conn;
    $offset = ($page - 1) * $count;
    ...
    if (empty($filter)) {
        ...
        $stmt = executeQuery($sql, ['count' => $count, 'offset' => $offset]);
    } else {
        $i = 0;
        ...
        $stmt = executeQuery($sql, $filter);
    }

    $records = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

    $totalRecords = countRecords('posts');
    return [
        'posts' => $records,
        'next_page' => $page * $count < $totalRecords ? $page + 1 : null,
